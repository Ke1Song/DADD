...
class iCIFAR100(CIFAR100):
    ...
    def concatenate(self, datas, labels):
        ...
        for i in range(1, len(datas)):
            con_data = np.concatenate((con_data, datas[i]), axis=0)  # 500张图片合成一个con_data
            con_label = np.concatenate((con_label, labels[i]), axis=0)  # labels[i]=labels[0]
        return con_data, con_label

    def getTestData(self, classes):
        datas, labels = [], []  # datas和labels的长度是classes的长度#label是classes的从起始到终止
        for label in range(classes[0], classes[1]):  # 第一个任务就是0-50  否则50-55，55-60等
            data = self.data[np.array(self.targets) == label]  # self.data.shape=50000x32x32x3 data.type=np.ndarray
            datas.append(data)  # self.targets的长度是10000，这里是targets是测试集每个label的类别
    ...
    def getTrainData(self, classes):
        datas, labels = [], []
        for label in range(classes[0], classes[1]):
            data = self.data[np.array(self.targets) == label]  # 获取第label个500张训练集图片
            datas.append(data)
            labels.append(np.full((data.shape[0]), label))  # 给数据打上标签
