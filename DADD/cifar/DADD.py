...
class DADD:
    ...
    def beforeTrain(self, current_task):
        ...
        if current_task == 0:
            ...
            classes = [self.numclass - self.task_size,
                       self.numclass]
    ...
    def protoSave(self, model, loader, current_task):  # self.protoSave(self.model, self.train_loader, current_task)
        ...
        labels = []
        model.eval()
        with torch.no_grad():  # images.shape=64x3x32x32
        ...
        labels = np.array(labels)  # before label reshape (390, 64)
        labels = np.reshape(labels, labels.shape[0] * labels.shape[1])  # after label  reshape (24960,)-24960=(390*64)
        features = np.array(features)  # before feature reshape 390x64x512
    ...
    def semanAug(self, features, y, labels, ratio):
        ...
        C = self.numclass
        A = features.size(1)
        weight_m = list(self.model.fc.parameters())[0]  # 512
    ...
    def classAug(self, x, y, mix_times=2):
        ...
        rot_x = torch.stack([torch.rot90(x, k, (2, 3)) for k in range(1, 4)], 1).view(-1, 3, 32, 32)
