...
class DADD:
    def __init__(self, args, file_name, feature_extractor, task_size, device):
        self.args = args
        self.numclass = self.args.fg_nc + 5
        ...
        # self.model = torch.load('50_model.pkl')
        # self.old_model = torch.load('50_model.pkl')
        self.cov = None
        self.prototype = None
        self.class_label = None
        self.old_model = None
        self.gradcam_layer = self.model.feature.layer4
        self.device = device
        ...
        self.class_set = list(range(args.total_nc))

    def beforeTrain(self, current_task):
        self.model.eval()
        if current_task == 0:
            classes = self.class_set[:self.numclass]
        else:
            classes = self.class_set[self.numclass - self.task_size:self.numclass]
        print(classes)
        self.trainfolder = self.data_manager.get_dataset(self.train_transform, index=classes, train=True)
        self.testfolder = self.data_manager.get_dataset(self.test_transform, index=self.class_set[:self.numclass],
                                                        train=False)
        self.train_loader = torch.utils.data.DataLoader(
            self.trainfolder, batch_size=self.args.batch_size, shuffle=True,
            drop_last=True, num_workers=8)
        self.test_loader = torch.utils.data.DataLoader(
            self.testfolder, batch_size=self.args.batch_size, shuffle=False,
            ...
            with GradCAM(self.model, self.model.feature.layer4) as gradcam:
                with GradCAM(self.old_model, self.old_model.feature.layer4) as gradcam_old:
                    ...
                    for _ in range(self.args.batch_size):
                        ...
                        temp = self.prototype[index[0]] + random.uniform(0.0, 0.15) * np.random.normal(0, 1, 512)
    ...
    def protoSave(self, model, loader, current_task):  # self.protoSave(self.model, self.train_loader, current_task)
        features = []
        labels = []
        model.eval()
        max_num = min(len(loader), 200)
        with torch.no_grad():  # images.shape=64x3x224x224
            for i, data in enumerate(loader):
                ...
                if feature.shape[0] == self.args.batch_size:  # feature.shape=64x512
                    labels.append(target.numpy())  # target.size=64         #feature[0].shape=512 feature.shape[0]=64
                    features.append(feature.cpu().numpy())
        labels_set = np.unique(labels)
        labels = np.asarray(labels)  # before label reshape (390, 64)
        labels = np.reshape(labels, labels.shape[0] * labels.shape[1])  # after label  reshape (24960,)-24960=(390*64)
        features = np.asarray(features)  # before feature reshape 390x64x512
        features = np.reshape(features, (features.shape[0] * features.shape[1], features.shape[2]))
        feature_dim = features.shape[1]
        # after feature reshape (24960, 512)
        prototype = []
        cov = []
        radius = []
        class_label = []
    ...
    def semanAug(self, features, y, labels, ratio):
        ...
        sigma2 = ratio * torch.bmm(torch.bmm(NxW_ij - NxW_kj, CV_temp.float()),
    ...
    def classAug(self, x, y, mix_times=2):
        ...
        rot_x = torch.stack([torch.rot90(x, k, (2, 3)) for k in range(1, 4)], 1).view(-1, 3, 224, 224)
